...
DESC_ANAL_OUTPUT = 'DESC_ANAL_OUTPUT'  # Directory for descriptive analysis outputs
...
def generate_evaluation_report():
    ...
    html.append('            <p>This report presents a comprehensive evaluation of the food waste analysis project, including descriptive analysis and predictive modeling. The evaluation covers data patterns, time series forecasting, regression analysis for yield prediction, and classification for efficiency categorization. The report includes performance metrics, visualizations, and interpretations to support decision-making and planning.</p>')
    html.append('        </div>')
    
    # Descriptive Analysis Section
    html.append('        <h2>1. Descriptive Analysis</h2>')
    html.append('        <div class="interpretation">')
    html.append('            <p>The descriptive analysis examines patterns and trends in the food waste collection and composting data, providing insights into collection efficiency, compost conversion rates, high-yield locations, and temporal trends.</p>')
    html.append('        </div>')
    
    # Collection Efficiency
    html.append('        <h3>1.1 Collection Efficiency</h3>')
    html.append('        <div class="visualization">')
    efficiency_dist_src = os.path.join(DESC_ANAL_OUTPUT, 'efficiency_distribution.png')
    efficiency_dist_dest = os.path.join(EVAL_DIR, 'efficiency_distribution.png')
    if copy_image(efficiency_dist_src, efficiency_dist_dest):
        html.append('            <img src="efficiency_distribution.png" alt="Efficiency Distribution">')
        html.append('            <p>Distribution of compost efficiency across collection sites, showing the range and frequency of efficiency values.</p>')
    else:
        html.append('            <p>Efficiency distribution visualization not available.</p>')
    html.append('        </div>')
    
    html.append('        <div class="visualization">')
    efficiency_trend_src = os.path.join(DESC_ANAL_OUTPUT, 'efficiency_trend.png')
    efficiency_trend_dest = os.path.join(EVAL_DIR, 'efficiency_trend.png')
    if copy_image(efficiency_trend_src, efficiency_trend_dest):
        html.append('            <img src="efficiency_trend.png" alt="Efficiency Trend">')
        html.append('            <p>Trend of compost efficiency over time, showing seasonal patterns and overall trajectory.</p>')
    else:
        html.append('            <p>Efficiency trend visualization not available.</p>')
    html.append('        </div>')
    
    html.append('        <div class="visualization">')
    daily_coll_src = os.path.join(DESC_ANAL_OUTPUT, 'daily_collection_trend.png')
    daily_coll_dest = os.path.join(EVAL_DIR, 'daily_collection_trend.png')
    if copy_image(daily_coll_src, daily_coll_dest):
        html.append('            <img src="daily_collection_trend.png" alt="Daily Collection Trend">')
        html.append('            <p>Daily patterns in food waste collection, showing variations across days of the week.</p>')
    else:
        html.append('            <p>Daily collection trend visualization not available.</p>')
    html.append('        </div>')
    
    # Compost Conversion
    html.append('        <h3>1.2 Compost Conversion</h3>')
    html.append('        <div class="visualization">')
    time_series_src = os.path.join(DESC_ANAL_OUTPUT, 'time_series_trends.png')
    time_series_dest = os.path.join(EVAL_DIR, 'time_series_trends.png')
    if copy_image(time_series_src, time_series_dest):
        html.append('            <img src="time_series_trends.png" alt="Time Series Trends">')
        html.append('            <p>Time series trends in compost production, showing patterns over time.</p>')
    else:
        html.append('            <p>Time series trends visualization not available.</p>')
    html.append('        </div>')
    
    html.append('        <div class="visualization">')
    eff_vs_vol_src = os.path.join(DESC_ANAL_OUTPUT, 'efficiency_vs_volume.png')
    eff_vs_vol_dest = os.path.join(EVAL_DIR, 'efficiency_vs_volume.png')
    if copy_image(eff_vs_vol_src, eff_vs_vol_dest):
        html.append('            <img src="efficiency_vs_volume.png" alt="Efficiency vs Volume">')
        html.append('            <p>Relationship between collection volume and compost efficiency, with high-yield threshold indicated.</p>')
    else:
        html.append('            <p>Efficiency vs volume visualization not available.</p>')
    html.append('        </div>')
    
    # Time-based Analysis
    html.append('        <h3>1.3 Time-based Analysis</h3>')
    html.append('        <div class="visualization">')
    monthly_coll_src = os.path.join(DESC_ANAL_OUTPUT, 'monthly_collection_trend.png')
    monthly_coll_dest = os.path.join(EVAL_DIR, 'monthly_collection_trend.png')
    if copy_image(monthly_coll_src, monthly_coll_dest):
        html.append('            <img src="monthly_collection_trend.png" alt="Monthly Collection Trend">')
        html.append('            <p>Monthly patterns in food waste collection, showing seasonal variations.</p>')
    else:
        html.append('            <p>Monthly collection trend visualization not available.</p>')
    html.append('        </div>')
    
    # Descriptive Analysis Interpretation
    html.append('        <div class="interpretation">')
    html.append('            <h3>Descriptive Analysis Insights</h3>')
    html.append('            <p>The descriptive analysis reveals several key insights:</p>')
    html.append('            <ul>')
    html.append('                <li>Collection efficiency varies significantly across sites, with opportunities for improvement at lower-performing locations.</li>')
    html.append('                <li>Temporal patterns show clear seasonal and weekly cycles in collection volumes and efficiency.</li>')
    html.append('                <li>There is a correlation between collection volume and compost yield, though with diminishing returns at very high volumes.</li>')
    html.append('                <li>High-yield locations share common characteristics that can be replicated at other sites.</li>')
    html.append('            </ul>')
    html.append('        </div>')
    
    # Time Series Model Evaluation (now section 2)
    html.append('        <h2>2. Time Series Forecasting Evaluation</h2>')
    ...
    # Regression Model Evaluation (now section 3)
    html.append('        <h2>3. Yield Prediction Model Evaluation</h2>')
    ...
    # Classification Model Evaluation (now section 4)
    html.append('        <h2>4. Efficiency Classification Model Evaluation</h2>')
    ...
    # Overall Conclusions
    html.append('        <h2>5. Overall Conclusions and Recommendations</h2>')
    html.append('        <div class="conclusion">')
    html.append('            <h3>Summary of Model Performance</h3>')
    html.append('            <p>The descriptive and predictive analyses developed for the food waste analysis project provide valuable insights into collection patterns, compost production forecasting, yield prediction, and site efficiency classification. These analyses support data-driven decision-making for operational planning and optimization.</p>')
    
    # Generate overall recommendations based on available metrics
    html.append('            <h3>Recommendations</h3>')
    html.append('            <ol>')
    html.append('                <li><strong>Collection Optimization:</strong> Focus collection efforts on high-yield days and locations identified in the descriptive analysis to maximize efficiency.</li>')
    ...
    html.append('        <div class="interpretation">')
    html.append('            <p>To further enhance the analytical capabilities and operational impact of these models, consider the following next steps:</p>')
    html.append('            <ol>')
    ...
    html.append('                <li>Create a feedback loop where operational changes are monitored and their effects are incorporated into model refinements.</li>')
    html.append('            </ol>')
    html.append('        </div>')
